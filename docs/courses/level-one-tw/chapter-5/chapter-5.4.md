...
```mermaid
flowchart LR
    A["If 節點<br/>True 分支<br/>14 筆處理中訂單"] --> B["Edit Fields 節點<br/>選擇欄位"]
    B --> B1["只保留 orderID"]
    B --> B2["只保留 employeeName"]
    B1 --> C["Airtable<br/>processingOrders 表格"]
    B2 --> C
    
    D["原始資料<br/>5 個欄位"] -.->|"過濾前"| A
    C -.->|"過濾後"| E["精簡資料<br/>2 個欄位"]
```
...
```mermaid
flowchart TD
    A["新增 Edit Fields 節點"] --> B["設定模式：手動映射"]
    B --> C["從輸入拖曳欄位"]
    C --> C1["拖曳 orderID"]
    C --> C2["拖曳 employeeName"]
    C2 --> D["設定包含其他輸入欄位：false"]
    D --> E["執行步驟"]
    E --> F["輸出只包含選定的兩個欄位"]
```
